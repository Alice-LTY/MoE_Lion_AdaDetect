...
from models.reconstructions.lion_adaptor import AdapterElement, AdapterRouter # Import mixture of experts (from lion_adaptor)
...
class Wide_Transformer(nn.Module):
    def __init__(
        ...
        self.adapter_ffn = AdapterRouter( # Lion_adaptor
            d_model=hidden_dim,
            bottleneck=hidden_dim, # Hidden_dim = 256
            dropout=dropout,
            num_adapters=moe_nume,  
            adapter_scalar="learnable_scalar",
            adapter_layernorm_option="none"
        )
        self.adapter_ffn_2 = AdapterRouter( # Lion_adaptor
            d_model=hidden_dim,
            bottleneck=hidden_dim, # Hidden_dim = 256
            dropout=dropout,
            num_adapters=moe_nume,
    ...
    def forward(self, src, pos_embed):
        ...
        for i in range (self.num_decoder):
            ...
            output_decoder = self.adapter_ffn(tgt)
            output_decoder = self.norms[2*i+2](output_decoder)

            output_decoder = self.adapter_ffn_2(output_decoder)
            output_decoder = self.norms[2*i+3](output_decoder)
            if self.return_intermediate:
                intermediate.append(output_decoder)
